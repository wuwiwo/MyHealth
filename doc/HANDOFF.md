...
| 章节 | 内容 | 什么时候看 |
|------|------|-----------|
| **§0** | 立即上手：跑起来 / **git 环境判定** / 三条纪律 / 改完必跑 | **接手第一件事** |
| §1–2 | 项目概览 / 版本状态与分支沿革 | 了解现状 |
| §3 | 完整模块清单（= 页面加载顺序） | 加模块前 |
| **§4** | 🎨 v2.1 设计体系：令牌速查 / 现成组件 / 护栏测试 | **写任何样式前** |
...
> **注意 1**：该写入在本环境的沙箱内会被静默丢弃（`exit=0` 但无效），必须以**绕过沙箱**的方式执行才生效。各平台机制不同，关键是**以文件是否真的出现为准**，别信命令的退出码。
>
> **注意 2**：**每次 `git push` / `fetch` 之后，`.git/refs/remotes/origin/` 目录本身可能会被删掉**（删除操作会生效，创建操作不会）—— 所以修复时**务必先 `mkdir -p` 再写**，写完用 `cat` 确认真有内容。曾因为漏了 `mkdir` 直接 `echo >` 而报 `No such file or directory`。
...
- [x] ~~**动作百科媒体待确认**~~ → **v2.1.26 已实测可访问**。Vercel 部署根目录 = `page/`，
  ...
  `MEDIA_BASE=''` 走 `media/<rel>` 相对路径，解析基准是 HTML 自身，线上线下一律正确，**无需改码**
- [ ] **云同步**：**v2.1.20 已修「载荷漏键」**（宠物/材料袋/宝珠/敌群进度/玩家技能此前完全不参与同步，往返实测 19 键丢 5），
  并加了 `test-sync-coverage` 守卫。~~多设备并发写入未压测~~ → **不适用**：dundun 2026-09-16 说明是个人使用，
  不存在多设备同时写入的场景（Blob 冲突合并策略较简单一事保留记录，见 §7.1）
- [x] ~~**4 个技能未入池**（`chargeup` / `doom` / `lastword` / `heal`）~~ → **v2.1.22 已全部入池**，
  正好接上 `ai.js:78-97` 既有的「队友低血治疗」与「Boss 残血放大招/特级负面」分支。
  实测副作用：**g14 胜率 38% → 3%**（g1~g13 仍 100%）；若觉得 g14 太硬，把 `lastword`/`doom` 摘出即可
- [x] ~~**技能等级只接了「攻击威力」**~~ → **v2.1.26 已接治疗/增益/状态持续/睡眠时长；v2.1.28 再补打湿幅度**。
  打湿通过**状态实例**传幅度（不动 wet 定义），故雨天场地仍固定（dundun 裁决：技能成长、场地固定）
  （打湿的魂防与命中、闪耀的降命）、增益幅度（战意灌注）、睡眠时长仍写固定值。
  ~~另记：无影拳的「5 连击」其实只打 1 次~~ → **v2.1.24（`b4f0467`）已实装 5 连击**（v2.1.26 同步本条）。
  **仍未做**：治疗量 / 状态幅度 / 增益幅度 / 睡眠时长仍是固定值（见 v2.1.26 施工记录）
...
- [x] ~~**`group-progress.js` 陈旧注释**~~ → **v2.1.26 已修**。正文 v2.1.21 改过，但第 10 行的
  「注册 `dh-group-progress` schema」仍是旧键名；已改为按**逻辑名** `groupProgress` 注册（物理键由 `store.js` 的 physKey 生成）
