...
### 新增功能
- ⚖️ 动作库支持等效重量（eqWeight）：自重/核心动作可设每次等效重量，自动计入容量
- ⏱️ 计时动作支持：unit 字段区分按次数('rep')或按秒数('sec')统计
- 📉 折算率 ratio 范围调整为 10~100%（原 1~100%）
- 容量公式：eqWeight × reps(或duration) × (ratio/100)
- 记录表单自动适配：选到 eqWeight 动作时隐藏重量网格，显示等效重量信息

### UI 调整
- 动作编辑器加「等效重量」输入和「计量单位」选择（次数/秒数）
- 动作库列表显示等效重量标签（如「⚖️ 0.5kg/次」）
- 训练记录列表 eqWeight 动作显示等效重量而非 0kg
- 训练记录列表按秒数动作显示「秒」而非「次」

## v1.7.5

### 新增功能
- 🔄 挑战自动模式：战斗 header 加「自动」按钮，开启后胜利自动进入下一关（2秒延迟）
- 💥 攻击打击动画：攻击方 lunge 前冲、被击方弹出伤害数字 + 颤抖

### UI 调整
- 旬奖励防御属性改为攻击的 1/5（如攻+90 → 防+18）
- 修正已补发的6月防御奖励（def 180→36，多出的144自动扣除）
