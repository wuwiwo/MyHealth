...
**裁决记录（2026-08-27 用户拍板）：**

1. **跨设备/跨时区的日/月键权威**：按 **A 本机本地日历先行**（用户确认云同步真实存在，但跨设备/时区场景尚未实测）；**B 云端统一时区权威正式列为 sync v5 议题**，届时若出现"跨时区双设备一天两次/漏掉重置"再升级。
2. **键升版后旧键删除 vs 保留**：**保留**（用户明确：升级后旧数据必须保留，辛苦不能丢）。实现折中：写新键成功后**旧键重命名为 `dh-<key>-v<n>-bak` 保留**（可回滚），但**读路径只读新键**，杜绝双源脏读；bak 键由 sync 正常备份。
3. **stats.js 月度重置委托是否纳入 S1**：**纳入**（Commit B2，用生产消费者验证 util）。
4. **S0 与 S1 是否双代理并行**：**并行**（文件集互斥，仅 index.html 由 S1 独占）。
