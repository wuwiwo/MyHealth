...
- `page/challenge.js`（summonRate 阶梯函数 / attemptSummon+canSummon 接入；面板阶梯文案由 v1.10.4 修正）
...
- 🩹 **删除训练记录同步扣除召唤资格（容量回撤守卫）**
  - 漏洞：召唤成功（pendingChallenge）后删除当日训练条目，容量缩水但资格保留，可"先凑容量召唤→删记录→白嫖挑战"
  - 修复：`canSummon()` 增加回撤守卫 —— 当 `total(=今日等效容量÷100) < todayUsed` 且召唤尚未开始时，撤销 pendingChallenge 资格并把 todayUsed 封顶到新 total
  - 触发时机：打开召唤面板/任何 canSummon 调用处，实时对齐；撤销时 toast 提示「检测到训练记录被删除，今日召唤资格已同步撤销」
  - 已结算的历史成绩不受影响（伤害是真实操作产生的）；已开始/已完成的当天不回溯
  - 附带修复：次数用完卡片的「可用 N/M 次」在容量缩水后可能显示负数 → Math.max(0) 钳制

### 设计说明

- 召唤次数与成功率阶梯完全由**当前真实训练量**推导，删记录 = 容量减少 = 资格同步缩水，与"多练多召唤"的正向激励闭环一致
...
- `page/challenge.js`（canSummon 容量回撤守卫 / 撤销 toast / 可用次数负数钳制）
- `page/utils.js`（APP_VERSION 1.9.10 → 1.9.11）
- `page/index.html`（cache-busting v16 → v17）
- `README.md`（副标题版本号 / 版本表新增 v1.9.11 行）
