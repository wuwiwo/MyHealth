...
> Personal Health Manager — 个人健身健康管理应用 v2.1.29
...
| **v2.1.23** | **2026-09-16** | **🎯 改正「区间」的驱动源并补齐其余 5 处**：查设计文档原文确认区间由**基础属性成长**决定，而宠物基础属性的成长线是**炼化**（炼化石，上限 R50/SR60/SSR80/UR100），**不是技能等级**（灵能——OQ-4 明确二者是两套材料体系）→ v2.1.22 用技能等级当驱动源是错的，本版收敛为单个 `skillRangeT(unit)`（OQ-8 曲线待定，线性占位，定案只改这一处）；给 `applySkillEffects` 注入 `ctx.sv`，让治疗/状态/增益三类通道也吃区间 → **圣光治愈、睡觉、打湿、闪耀、战意灌注 5 处补齐**；**顺带修一个真 bug**：`castSkill` 应用状态时没透传 `modsPct`/`data`，会让打湿的幅度在实战中被静默丢掉；缓存版本对齐（v83）** | `doc/changelog-v2.1.md` |
...
| **v2.1.29** | **2026-09-18** | **🗺️ 敌群由 18 大关扩到 21 大关（新增 g19 试炼·太虚 / g20 试炼·混元 / g21 试炼·无极，共 210 关）；难度沿用 g16+ 的缓斜率 GROUP_TAIL_POW_EXT=1.24，不引入新拐点；**消除大关数硬编码**——此前 `GROUP_STAGE_NAMES` 与生成循环 `lg <= 18` 各自写死，只加名字表会**静默失败**（关卡数不变、测试不挂），现大关数由名字表派生为 `GROUP_MAX`，扩关只需加一行名字；测试两处写死（大关数 `=== 18`、超限大关数 `=== 6`）也改为派生；缓存版本对齐（v89）** | `doc/changelog-v2.1.md` |

> 当前版本：**v2.1.29**（设计规范：`doc/design-tokens-v2.1.md`）
